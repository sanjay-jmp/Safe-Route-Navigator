        ...
        with driver.session() as session:
            ...
            for u, v, key, data in G.edges(keys=True, data=True):
                ...
                if 'geometry' in props:
                    # Flatten the list of coordinate pairs into a single list of floats
                    ...
                    del props['geometry'] # Remove the Shapely object
                # --- END FIX ---
            ...
            for i in tqdm(range(0, total_relationships, BATCH_SIZE), desc="Importing relationships"):
                ...
                try:
                    ...
                    # You can also print the batch content for debugging
                    # print(f"Problematic batch data (first few items): {batch[:3]}")
                    raise # Re-raise to stop if a batch fails completely
            ...
            session.run("CREATE INDEX ON :Intersection(osmid)")
            session.run("CREATE INDEX ON :Intersection(latitude, longitude)")
